...
class VoiceSettings:
    ...
    def from_env(cls) -> "VoiceSettings":
        ...
        settings = cls(
            ...
            confidence_gate_disabled=_enabled("VOICE_CONFIDENCE_GATE_DISABLED"),
        ...
        if settings.asr_provider == "azure" and not settings.confidence_gate_disabled:
            raise ConfigurationError(
                "Azure ASR exposes a constant confidence in this integration; "
                "set VOICE_CONFIDENCE_GATE_DISABLED=1 explicitly or select a calibrated provider"
            )
        return settings

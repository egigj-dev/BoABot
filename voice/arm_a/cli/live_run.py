...
from voice.shared.turn_client import TurnClient
from voice.shared.tts.azure_tts import AzureTTS
...
def _assert_real_components(
    ...
    assert not isinstance(asr, __import__("voice.arm_a.asr.fake_adapter", fromlist=["FakeStreamingASR"]).FakeStreamingASR), "FakeStreamingASR is forbidden in live runs"
    assert not isinstance(tts, __import__("voice.shared.tts.fake_tts", fromlist=["FakeTTS"]).FakeTTS), "FakeTTS is forbidden in live runs"
    assert not isinstance(turn_service, __import__("voice.shared.mock_turn", fromlist=["ScriptedTurnService"]).ScriptedTurnService), "ScriptedTurnService from mock_turn is forbidden in live runs"

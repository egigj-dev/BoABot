...
ROOT = HERE.parent
_rates_path = ROOT / "rate_tables.jsonl"
...
def main() -> None:
    paths = sys.argv[1:] if len(sys.argv) > 1 else [str(HERE / p) for p, _ in ALL_SETS]
    # Map labels
    label_map = {p: lbl for p, lbl in ALL_SETS}
    label_map.update({str(HERE / p): lbl for p, lbl in ALL_SETS})

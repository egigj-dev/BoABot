...
from core.callcenter import (_HANDOFF_THRESHOLD, _PROBE_LABELS, _PROBE_VECTORS,
                             _SECRET_FAST_RE, decide)
from core.retrieve import model

HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
PHRASES = HERE / "handoff_phrases.jsonl"  # Fixed blind phrase bank.
STRATIFIED_SPLIT = HERE / "handoff_split.json"  # Previous row-stratified split.
GROUPED_SPLIT = HERE / "handoff_split_grouped.json"  # Leakage-free family split.
ARTIFACT = ROOT / "handoff_probe.json"  # Exported grouped winner data.
...
def main() -> None:
    ...
    production_texts = [callcenter.restore_diacritics(text) for text in texts]
    production_vectors = np.asarray(model().encode(
        production_texts, normalize_embeddings=True, batch_size=64,
        show_progress_bar=False,
    ), dtype=np.float32)
    embedding_cache = {text: vector for text, vector in zip(production_texts, production_vectors)}
    original_encoder = callcenter._encode_question
    callcenter._encode_question = lambda text: embedding_cache[text]
    try:
        decisions = [decide(texts[row_id], "", []) for row_id in grouped_test]
        production = np.asarray([decision.handoff for decision in decisions])
    finally:
        callcenter._encode_question = original_encoder
    expected = np.asarray([
        bool(_SECRET_FAST_RE.search(production_texts[row_id]))
        or (not callcenter._is_informational_banking_query(production_texts[row_id])
            and (score := callcenter._probe_score(production_vectors[row_id])) is not None
            and score >= _HANDOFF_THRESHOLD)
        for row_id in grouped_test
    ])
    if not np.array_equal(production, expected):
        for row_id, actual, wanted, decision in zip(
                grouped_test, production, expected, decisions):
            if actual != wanted:
                print(f"Production mismatch: expected={bool(wanted)} actual={bool(actual)} "
                      f"reason={decision.reason} intent={rows[row_id]['intent']} "
                      f"text={rows[row_id]['text']}")
        raise RuntimeError("Production decide() disagrees with grouped winner")
    print(f"Production verification: decide() matches Method B on "
          f"{len(grouped_test)}/{len(grouped_test)} grouped test rows after the production "
          f"diacritic-restoration preflight.")
    paths = Counter(decision.reason or "unclassified_answer_path" for decision in decisions)
    print(f"Production HANDOFF count: {int(production.sum())}/{len(production)}")
    print("Production triggering paths: " + ", ".join(
        f"{reason}={count}" for reason, count in sorted(paths.items())))
